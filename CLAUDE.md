...
- `src/video.h` / `src/video.cpp` — **WebM 動画再生** (`wamsoft/movie-player` + libvpx + libvorbis/libopus)。 CMake オプション `JSENGINE_USE_MOVIE_PLAYER` (default ON) で有効化、 FetchContent で movie-player を取得。 ローカル checkout を使いたい場合は `USE_LOCAL_MOVIE_PLAYER=ON` + `JSENGINE_LOCAL_MOVIE_PLAYER_PATH=<dir>` で `add_subdirectory` 経由 (krkrz の同 submodule に直接修正を入れたいとき用)。
- グローバル `MoviePlayer` クラスを公開: `new MoviePlayer("file.webm", {loop, volume, colorFormat})` / `play/pause/resume/stop/seek` / `width/height/duration/currentTime/paused/ended/loop/volume` / `data` (RGBA ArrayBuffer)。
- `colorFormat` オプションで二系統の経路を選択:
  - `"rgba"` (default): movie-player の **旧 API** (`SetOnVideoDecoded` + `DestUpdater`) を使い、 decoder の packed RGBA を host バッファに **直接書込ませる高速経路** (余計な memcpy 無し)。 `data` getter で 1 plane の ArrayBuffer。
  - `"i420"` / `"nv12"` / `"nv21"`: movie-player の **新 API** (`SetOnVideoDecodedPlanes` + `VideoFrameInfo`) を使い、 decoder native の YUV plane を生で受け取って内部に row-by-row でコピー保持 (stride 補正)。 `planes` / `getPlane(i)` で `{width,height,data}` を取得。 shader 側で YUV→RGB 変換することで GPU 負荷分散可能。 plane.width は coded (stride-padded) 幅で `videoWidth` より大きい場合あり (VP9 で 1920→1984 など)、 shader 側で UV 範囲を制限する。
- 内部の `SDLAudioSink` (IAudioSink 実装) が SDL_AudioStream へ PCM を流す (movie-player は内部 audio engine 非搭載なので host が sink を渡す設計)。 `Enqueue` は data を SDL に即コピー + param を consumed queue に積んで返却し、 decoder slot 詰まりを回避 (Android 推奨パターンと同じ)。
- `HTMLVideoElement` (sysinit.js) はネイティブ MoviePlayer のラッパとして動き、 `video.src = "..."` で内部 player 生成、 `video.play/pause/loop/volume/currentTime/videoWidth/videoHeight/data` 等をブラウザ互換で公開する (`gl.texImage2D(..., video)` も MoviePlayer と同経路でテクスチャ化、 RGBA mode 固定)。
- **getter/setter は `JS_NewCFunction` の標準 4 引数シグネチャ (ctx, this_val, argc, argv) が必須**: 3 引数版を渡すと `val` がガベージになり SEGV。
- **movie-player 側の API 構成 (上流 wamsoft/movie-player を直接修正)**: 旧 `SetOnVideoDecoded(OnVideoDecoded)` (DestUpdater 経由、 packed 専用、 高速) + 新 `SetOnVideoDecodedPlanes(OnVideoDecodedPlanes)` (VideoFrameInfo 経由、 YUV 含む全 format 対応) の **二口並存**。 同時利用は不可 (内部 callback slot を上書きするので最後に呼んだ方のみ有効)。 host は用途に応じて選ぶ。
- `data/main.js` の Demo 13 (`\` キー、 Shift+\\ で native ⇄ sim 切替) が利用例。
...
- `samples/vrm_starter/` — **VRM ベースシステム (vrmkit) + ひな型一式**。詳細は `samples/vrm_starter/README.md`。
  ...
  - **VRMA 対応**: `lib/three-vrm-animation.module.min.js` (@pixiv/three-vrm-animation v3.5.1、three-vrm と同版) を同梱。`loadVRM` が `VRMLookAtQuaternionProxy` を scene に追加してから `createVRMAnimationClip` する順序が必須。
  - ハマりどころ: (1) three-vrm の正規化リグは **バインド時の向き基準** (VRM0 = -Z 正面) なので、腕を下ろす等の定数ポーズ回転は `vrm.scene` ローカル系で腕方向を実測して符号を決める (`VRMActor._armSign`)。`rotateVRM0` は `vrm.scene.rotation.y = π` を入れるだけで正規化リグの軸は変わらない。(2) カメラフレーミングでボーンのワールド座標を読む前に `updateWorldMatrix(true, false)` が必要 (シーン配置直後はレンダ前で行列が古い)。(3) lib/ の ESM を main.js から直接 loadModule すると vrmkit 内の import と二重インスタンス化するので、必ず vrmkit 経由で使う。
  - REPL フック: `globalThis.__vrmstarter` に app / switchMode を公開。`-replfile` + `captureScreen` で両モードの自動検証が可能。
- `samples/anime25d/` — **Anime2.5DRig (852wa/Anime2.5DRig, MIT) の移植**。パーツ分け PSD を自動リギングして動かす 2.5D アバター (メッシュワープ + ステンシル瞳クリップ + 2重バネ髪物理)。詳細は `samples/anime25d/README.md`。
  - `lib/` は上流無改変 (ag-psd / rigger / genericparts)。`rig25d/` が組み込みライブラリ 3 分割: `loader.js` (データ作製部 `Rig25D` — PSD→rig + `.rig` バイナリキャッシュ)、`avatar.js` (表示機能部 `Avatar25D`)、`ui_panel.js` (テスト操作 UI `UIPanel25D`)。マイク口パク / カメラ追従 (MediaPipe) は機能カット、マウス追従は残置。
  - **buildRig は QuickJS だと 1280px PSD で約 1 分** (V8 なら数秒の処理)。`Rig25D.loadRig` が PSD サイズ + 先頭 64KB FNV ハッシュで照合する `.rig` キャッシュを PSD の隣に書き、2 回目以降は即起動。
  - ハマりどころ: (1) `new Canvas2D(w,h)` 自体が 2d コンテキストで `getContext("2d")` は無い。(2) ag-psd は readPsd 前に `agPsd.initializeCanvas()` で純 JS 実装 (`{width,height,data}` を返す createImageData) に差し替える (sysinit の canvas シム経由の auto-init に依存しない)。(3) rig パーツのテクスチャは `{width,height,data}` を `UNPACK_PREMULTIPLY_ALPHA_WEBGL=true` で texImage2D に渡す (straight のままだと premultiplied blend と不整合で白フチが出る)。
  - モデル PSD は権利上リポジトリ未同梱 (`psd/` を gitignore)。起動後に左上「画像を読み込む」ボタン → **`fs.showOpenFileDialog(filters)`** (このサンプル用に新設した SDL_ShowOpenFileDialog の Promise バインディング。コールバックは別スレッド→ `JsEngine::update` 冒頭の `file_dialog_drain` で resolve) で任意の場所の PSD を開く。キャッシュは選択 PSD の隣に書く。
  - **既知の制限**: リグ生成中 (約 1 分) は同期処理でイベントポンプが止まりウィンドウが応答なしになる。改善計画: buildRig のレイヤー単位チャンク実行 / Worker (別スレッド QuickJS) / Node での .rig 事前生成 (rigger.js は Node 互換)。
  - REPL フック: `globalThis.__anime25d` (isReady/state/avatar/panel/rigInfo/loadPsd)。
