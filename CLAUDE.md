...
- `src/webaudio.h` / `src/webaudio.cpp` — Web Audio API bindings. AudioContext + AudioBufferSourceNode + GainNode (フェード対応: `gain.value`, `setValueAtTime`, `linearRampToValueAtTime`, `exponentialRampToValueAtTime`, `cancelScheduledValues`) + AudioBuffer + `decodeAudioData(arrayBuffer)` を提供。`source.connect(gain).connect(ctx.destination)` のグラフに加え、**gain → gain → ... → destination の多段チェーンも対応** (master/BGM/SE 等の階層型ボリューム制御用)。source の実効ボリュームは `source.localVolume × g1.cachedValue × g2.cachedValue × …` で、上流 gain の値変更時は `JsAudioGain::inputGains` を辿って影響範囲のすべての source に再適用する (`apply_volume_to_subtree`)。`createBufferSource(path)` の path 引数はファイル直接ロードする jsengine 拡張で従来コードと互換。`webaudio_update(deltaMs)` を毎フレーム呼び `AudioContext.currentTime` を進めて gain ramp を反映 (旧 `webaudio_gc()` は互換用ラッパ)。**既知の課題**: JS 側で生成した GainNode は JS 参照が切れると finalizer で `s.connectedGain` が null 化されチェーンが切れる。一発鳴らしの SE は呼び出し元で参照保持しないと「master/グループ音量が効かなくなる」現象が出る。本来は AudioEngine 側にサウンドグループ概念を持たせ、source が直接グループに属する設計に組み直すべき (今後の課題、`audio-group-redesign` メモ参照)。
...
- `data/framework/assets_ext.js` — `Assets.preloadAudio({alias: path})` で MP3/WAV/OGG/FLAC/Opus を fetch+decodeAudioData する自前プリローダ (PIXI v7 の URL リゾルバはブラウザ標準 URL コンストラクタ依存で jsengine の URL シムと非互換のため、音声は PIXI.Assets を経由しない)。フォントは PIXI.Assets の LoadParser として `Canvas2D.loadFont` を呼び family 名を返す。`Assets.audioContext` / `Assets.masterGain` / `Assets.bgmGain` / `Assets.seGain` を公開し、出力グラフは `source → (localGain) → bgmGain or seGain → masterGain → destination`。`Assets.play(alias, opts)` で即時再生ヘルパー、`Assets.getAudio(alias)` で AudioBuffer 取得。
- `data/framework/sound_manager.js` — BGM クロスフェード / ダッキング / SE 単発再生のヘルパー。`SoundManager.playBgm(alias, {fadeIn, volume})` で同 alias は no-op、別 alias ならクロスフェード。`stopBgm(fadeOut)` / `pauseBgm(level, dur)` / `resumeBgm(dur)` でダッキング。`playSe(alias, opts)` は SE グループ (`Assets.seGain`) 経由で再生。`tick()` を毎フレーム呼んで再生終了 SE エントリを掃除する (チェーン保持用の `activeSounds` リスト)。**現状の制約**: 一発鳴らしの SE は呼び出し側で参照保持しないと QuickJS GC で localGain が回収されチェーンが切れ「マスター/グループ音量が SE に効かなくなる」現象が出る。本質的な解決は AudioEngine 側にサウンドグループ概念を持たせる設計変更が必要 (今後の課題、`audio-group-redesign` メモ)。
...
- Demo 11 (`-` キー): SceneManager / Input / Assets / SoundManager を組み合わせた**フレームワーク事例**。Boot (Assets ロード) → Title → Menu → Settings / Game → Pause の遷移サンプル。RPG メニュー型 UI、push/pop/replace の使い分け実演、Settings で Master/BGM/SE 3 スライダー (pixi.ui Slider、上下で行移動、左右で値変更、localStorage 永続化)、Game でアクション抽象化 (キーボード/Gamepad)、Pause で `pauseBelow` モーダル + BGM ダッキング。BGM (`data/bgm/title.wav`, `data/bgm/game.wav`) と SE (`data/se/select|confirm|cancel|fire|pause.wav`) は `tools/gen_audio_assets.py` で合成生成 (Python 標準ライブラリのみ、wave + math)。実体は `data/demos/demo11_scene_showcase.js`。
